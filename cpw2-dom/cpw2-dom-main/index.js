...
function background(){
     ...
     */
    var fundo = document.querySelector('body').style.background = "#BECCC3";
}
...
function show(){
     ...
     var result = document.getElementById("result").innerHTML = (nome + " " + sobrenome);
...
function search(){
    ...
    data.forEach(person => {
         if(person.name.toLowerCase().substr(0, nome.length).indexOf(`${nome.toLowerCase()}`) > -1){
            x += `<div>${person.name}</div>`;
        }});
